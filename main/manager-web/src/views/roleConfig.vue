              ...
              <div class="form-content">
                <div class="form-grid">
                  <div class="form-column">
                    ...
                    <el-form-item :label="$t('roleConfig.agentType') + '：'">
                      <el-radio-group v-model="form.agentType" @change="handleAgentTypeChange">
                        <el-radio label="normal">{{ $t('roleConfig.normalAgent') }}</el-radio>
                        <el-radio label="promptx">{{ $t('roleConfig.promptxAgent') }}</el-radio>
                      </el-radio-group>
                    </el-form-item>
                    <el-form-item :label="$t('roleConfig.roleTemplate') + '：'" v-if="form.agentType === 'normal'">
                    ...
                    <el-form-item :label="$t('roleConfig.promptxRole') + '：'" v-if="form.agentType === 'promptx'">
                      <promptx-role-selector
                        v-model="form.promptxRoleId"
                        @change="handlePromptXRoleChange"
                      />
                    </el-form-item>
                    ...
                    <el-form-item :label="$t('roleConfig.roleIntroduction') + '：'">
                      <el-input
                        ...
                        rows="8"
                        resize="none"
                        :placeholder="form.agentType === 'promptx' ? '【PromptX模式】系统提示词将在运行时自动从角色定义加载，无需手动填写' : $t('roleConfig.pleaseEnterContent')"
                        v-model="form.systemPrompt"
                        :readonly="form.agentType === 'promptx'"
                        :maxlength="form.agentType === 'promptx' ? null : 2000"
                        :show-word-limit="form.agentType !== 'promptx'"
                        class="form-textarea"
                        :class="{ 'readonly-textarea': form.agentType === 'promptx' }"
                      />
                    </el-form-item>
...
export default {
  ...
  components: { 
    ...
    'promptx-role-selector': PromptXRoleSelector 
  },
  data() {
    return {
      ...
      form: {
        agentCode: "",
        agentName: "",
        agentType: "normal", // 智能体类型: normal | promptx
        promptxRoleId: "", // PromptX角色ID
        promptxRoleName: "", // PromptX角色名称
        promptxRoleDescription: "", // PromptX角色描述
        promptxRoleSource: "", // PromptX角色来源
        ttsVoiceId: "",
        chatHistoryConf: 0,
        systemPrompt: "",
        summaryMemory: "",
      ...
      // 缓存不同类型下的特有字段，避免切换时丢数据
      promptxSnapshot: null,
      normalSnapshot: null,
      // 功能状态
      featureStatus: {
  ...
  methods: {
    ...
    handleAgentTypeChange(newType) {
      // 切换智能体类型时缓存当前类型特有字段，再恢复目标类型缓存，避免数据丢失
      if (newType === 'promptx') {
        // 先缓存普通智能体的可编辑提示词
        this.normalSnapshot = {
        ...
        // 如果之前用过PromptX，恢复缓存；否则清空以便重新选择
        if (this.promptxSnapshot) {
          this.form.promptxRoleId = this.promptxSnapshot.promptxRoleId || '';
          this.form.promptxRoleName = this.promptxSnapshot.promptxRoleName || '';
          this.form.promptxRoleDescription = this.promptxSnapshot.promptxRoleDescription || '';
          this.form.promptxRoleSource = this.promptxSnapshot.promptxRoleSource || '';
          this.form.systemPrompt = this.promptxSnapshot.systemPrompt || '';
        } else {
          this.form.promptxRoleId = '';
          this.form.promptxRoleName = '';
          this.form.promptxRoleDescription = '';
          this.form.promptxRoleSource = '';
          this.form.systemPrompt = '';
        }
      } else {
        // 缓存PromptX特有字段和自动生成的系统提示词
        this.promptxSnapshot = {
          promptxRoleId: this.form.promptxRoleId,
          promptxRoleName: this.form.promptxRoleName,
          promptxRoleDescription: this.form.promptxRoleDescription,
          promptxRoleSource: this.form.promptxRoleSource,
          systemPrompt: this.form.systemPrompt,
        };

        // 切回普通智能体时恢复之前手填的提示词/记忆
        this.form.promptxRoleId = '';
        this.form.promptxRoleName = '';
        this.form.promptxRoleDescription = '';
        this.form.promptxRoleSource = '';
        this.form.systemPrompt = this.normalSnapshot?.systemPrompt || '';
        if (this.normalSnapshot?.summaryMemory !== undefined) {
          this.form.summaryMemory = this.normalSnapshot.summaryMemory;
        }
      }
    },
    async handlePromptXRoleChange(roleData) {
      console.log('PromptX角色选择:', roleData);

      // 保存角色信息
      this.form.promptxRoleId = roleData.roleId;
      this.form.promptxRoleName = roleData.roleName;
      this.form.promptxRoleDescription = roleData.roleDescription;
      this.form.promptxRoleSource = roleData.roleSource;

      // ✅ 新实现：系统提示词留空，运行时由action工具自动加载
      // 不再调用generateSystemPrompt API
      this.form.systemPrompt = "";

      // 提示用户
      this.$message.success({
        message: `已选择角色「${roleData.roleName}」，系统将在运行时自动加载角色定义`,
        showClose: true,
        duration: 3000
      });
    },
      ...
      Api.agent.getDeviceConfig(agentId, ({ data }) => {
        if (data.code === 0) {
          this.form = {
            ...this.form,
            ...data.data,
            agentType: data.data.agentType || 'normal',
            promptxRoleId: data.data.promptxRoleId || '',
            promptxRoleName: data.data.promptxRoleName || '',
            promptxRoleDescription: data.data.promptxRoleDescription || '',
            promptxRoleSource: data.data.promptxRoleSource || '',
            model: {
  ...
  async mounted() {
    ...
    this.fetchTemplates();
    // 预加载PromptX角色列表
    this.$store.dispatch('promptxAgent/fetchRoles');
